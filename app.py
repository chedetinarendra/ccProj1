...
config = {
  'host':'sushma08.mysql.database.azure.com',
  'user':'sushma@sushma08',
  'password':'Pinky@29',
  'database':'guest',
...
def get_https_url(item):
    return url_for(item, _external=True, _scheme='https')
    ...
    if request.method == 'POST':
        ...
        if hdata.filename == '' or tdata.filename == '' or pdata.filename == '' :
            ...
            for i,row in csvData.iterrows():
                ...
                value = (row['HSHD_NUM'],row['L'],row['AGE_RANGE'],row['MARITAL'],row['INCOME_RANGE'],row['HOMEOWNER'],row['HSHD_COMPOSITION'],row['HH_SIZE'],1)
                print(value)
            ...
            csvData = pd.read_csv(file_path,names=col_names,header=0)
            for i,row in csvData.iterrows():
                query='INSERT INTO transactions (BASKET_NUM,HSHD_NUM,PURCHASE_,PRODUCT_NUM,SPEND,UNITS,STORE_R,WEEK_NUM,YEAR) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)'
                value = (row['BASKET_NUM'],row['HSHD_NUM'],row['PURCHASE_'],row['PRODUCT_NUM'],row['SPEND'],row['UNITS'],row['STORE_R'],row['WEEK_NUM'],row['YEAR'])
                cur.execute(query,value)
                conn.commit()
            #Products data
